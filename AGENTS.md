...
- `src/layout`：主布局和大屏布局。
- `src/views/systemCore`：业务页面，当前重点参考 `collaboration/coordinate`、`collaboration/tracking`、`operations/control`。
- `src/api/system`：业务接口封装和类型定义。
...
- 主题色使用 Element Plus CSS 变量；业务状态色集中映射，避免散落在模板和样式中。
- 后续只调整主题色风格时，页面布局风格保持不变，优先修改 `src/settings.ts` 的 `theme` 默认值和必要的主题变量消费点，避免重写页面结构、卡片层级、间距和组件 API。
...
- 指令在 `src/directive/permission`，已全局注册；新增按钮请参照 `collaboration/briefing`、`collaboration/config` 等页面的现有用法补齐权限标识。
...
- 接口按模块放在 `src/api/system/<模块>/index.ts`，类型放同目录 `types.ts`（`XxxVO` 列表项、`XxxForm` 表单、`XxxQuery` 查询参数）。
...
1. **禁止滥用 any**：除与老旧系统交互等不可抗力外，所有声明必须定义明确的 TypeScript 类型。新增接口必须在 `src/api/system/<模块>/types.ts` 中声明。
