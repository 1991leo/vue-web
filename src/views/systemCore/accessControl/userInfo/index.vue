<template>
	<div class="p-2 page-layout-fill">
		<el-row :gutter="20" class="page-split-row">
			<!-- 区域树 -->
			<el-col :lg="4" :xs="24" class="page-split-tree-col">
				<el-card shadow="hover" class="page-split-tree-card">
					<el-input v-model="deptName" placeholder="请输入区域名称" prefix-icon="Search" clearable />
					<el-tree
						ref="deptTreeRef"
						class="mt-2"
						node-key="id"
						...
						:default-expanded-keys="setExpandedKeys"
						highlight-current
						@node-click="handleNodeClick"
					/>
				</el-card>
			</el-col>
			<el-col :lg="20" :xs="24" class="page-split-main-col">
				<transition
				...
				<el-card shadow="hover" class="page-split-table-card">
					<template #header>
						<el-row :gutter="10">
							<el-col :span="1.5">
					...
					>
						...
						<el-table-column v-if="columns[3].visible" key="deptName" label="区域" align="center" prop="deptName" />
						<el-table-column
...
const columns = ref<FieldOption[]>([
	...
	{ key: 3, label: `区域`, visible: true, children: [] },
...
const getDeptTree = async () => {
	...
	await nextTick()
	setExpandedKeys.value = enabledDeptOptions.value.map((item) => item.id)
}
...
const handleAuthRole = (row: UserVO) => {
	...
	router.push("/systemCore/accessControl/user-auth/role/" + userId)
}
