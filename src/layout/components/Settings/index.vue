...
// 是否暗黑模式：使用显式状态，避免浏览器/系统夜间模式自动给 html 添加 dark class。
const isDark = ref<boolean>(defaultSettings.dark)

const applyDarkMode = (enabled: boolean) => {
	isDark.value = enabled
	settingsStore.dark = enabled
	document.documentElement.classList.toggle("dark", enabled)
	localStorage.setItem("useDarkKey", enabled ? "dark" : "light")
	settingsStore.sideTheme = enabled ? SideThemeEnum.DARK : sideTheme.value
}

const toggleDark = (val: string | number | boolean) => {
	applyDarkMode(Boolean(val))
}

applyDarkMode(defaultSettings.dark)
...
const saveSetting = () => {
	...
	const settings = useStorage<LayoutSetting>(storageKey, defaultSettings)
	...
	// 注释掉保存 theme 到本地存储的逻辑，使其始终采用系统默认配置
	// settings.value.theme = storeSettings.value.theme;
	// 注释掉保存 navType 到本地存储的逻辑，使其始终采用系统默认配置
...
const resetSetting = () => {
	...
	const storageKey = `${defaultSettings.storageNamespace || ""}layout-setting`
	useStorage<any>(storageKey, null).value = null
